...
namespace AchSystem
{
    ...
    /// </summary>
    public class BankKBT : Bank
    {
        public BankKBT() { }

        public BankKBT(string bank, int bankid)
            : base(bank, bankid)
        {
            ...
            this.SettlementFileName = this.SettleFileDirectory + bank + @"\MERITUS_" + DateTime.Now.ToString("yyyyMMddHHmm") + ".tmp";
        ...
        public bool CreateSettlementFile(string RecordType, DateTime processDate, DateTime cuttOffTime)
        {
            try
            {
                ...
                prms.Add(new SqlParameter("@BankID", this.BankID));
                //prms.Add(new SqlParameter("@Date", "3/25/2010")); 
                    ...
                    if (LastBatchID != DataLayer.Field2Int(dr["BatchID"]) || LastBatchFileID != DataLayer.Field2Int(dr["FileBatchID"]))
                    {
                        if (LastBatchID != 0) //If this is not the first record
                        {
                            ...
                            switch (batch.Secc)
                            {
                                case "TEL":
                                case "WEB":
                                    // Mark Berker 03/30/2010 this.CreateBatchOffsetWEBTEL(file, batch, ref DetailCount); 
                                    break;
                                default:
                                    // Mark Berker 03/30/2010 this.CreateBatchOffsetDetailRecord(batch, ref DetailCount);
                                    break;
                            }
                        }
                        ...
                        //if (DataLayer.Field2Bool(dr["HighRisk"]))
                        //{
                        //    batch.CompanyName = "iStream";
                        //    batch.DiscretionaryData = "11302024234";//demo "11302037922";
                        //}
                        //else
                        //{
                        //    batch.CompanyName = "iStream";
                        //    batch.DiscretionaryData = "11302021892";//demo"11302037914";
                        //}

                        batch.CompanyName = "iStream";
                        batch.DiscretionaryData = DataLayer.Field2Str(dr["LocationID"]);
                ...
                while (dr.Read())
                {
                    if (Description != DataLayer.Field2Str(dr["Description"]))
                    {
                        ...
                        batch.CompanyName = "iStream";
                        /* DO NOT CHANGE BELOW - MARK NGUYEN 1/5/2011 */
                        batch.DiscretionaryData = "40025108141"; //"11302926258"; //Meritus Merchant Invoicing DataLayer.Field2Str(dr["LocationID"]);
                        //batch.DiscretionaryData = DataLayer.Field2Str(dr["LocationID"]);
