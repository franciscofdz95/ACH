...
{
    /// <summary>
    /// Summary description for BankAptPay.
    /// </summary>    
    public class BankAptPay : Bank
    {
        public BankAptPay() { }

        public BankAptPay(string bank, int bankid)
            : base(bank, bankid)
        {
            (this.SettleFileDirectory + bank).TryCreateDirectory((directory) => MessageBox.Show("Cannot create directory!\n " + directory));
        }

        public override bool CreateSettlementFile(DateTime Dt)
        {
            this.CompanyID = "5492721102";      //TODO_RV
            if (ConfigurationManager.AppSettings.AllKeys.Contains("AptPayCompanyID"))
                this.CompanyID = ConfigurationManager.AppSettings["AptPayCompanyID"].ToString();
        ...
        public bool CreateSettlementFile(string RecordType, DateTime processDate, DateTime cuttOffTime)
        {
            try
            {
                ...
                prms.Add(new SqlParameter("@RecordType", RecordType));
                ...
                var fileNamePattern = "ach-inbound-{0:yyyyMMddHHmmss}.txt";
                if (ConfigurationManager.AppSettings.AllKeys.Contains("AptPaySettlementFileName"))
                    fileNamePattern = ConfigurationManager.AppSettings["AptPaySettlementFileName"].ToString();

                this.SettlementFileName = this.SettleFileDirectory + this.BankName + @"\" + string.Format(fileNamePattern, processDate);
                ...
                while (dr.Read())
                {
                    if (LastBatchID != DataLayer.Field2Int(dr["BatchID"]) || LastBatchFileID != DataLayer.Field2Int(dr["FileBatchID"]))
                    {
                        ...
                        batch.OriginatingDFI = this.ImmediateDestination.Substring(1, 8);
