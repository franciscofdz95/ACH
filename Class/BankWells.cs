...
{
    /// <summary>
    /// Summary description for BankChase.
    /// </summary>
    public class BankWells : Bank
    {
        public BankWells() { }

        public BankWells(string bank, int bankid)
            : base(bank, bankid)
        {
        ...
        public override bool CreateSettlementFile(DateTime Dt)
        {
            ...
            //    for (var i = 2; i > 0; i--)
            //    {
            //        CreateSettlementFile("ALL",DateTime.Now.AddDays(-i), cuttOffTime.AddDays(-i - 1));
            //        cuttOffTime = DateTime.Now;
            //    }
            //}
            //// If this action is not  on Monday, the second parameter always will be avoid it by the SPs
            //return CreateSettlementFile("ALL",DateTime.Now, DateTime.Now.AddDays(-1));
        ...
        public bool CreateSettlementFile(string RecordType, DateTime processDate, DateTime cuttOffTime)
        {
            try
            {
                ...
                if (!Directory.Exists(this.SettleFileDirectory + this.BankName))
                {
                    try
                    {
                        Directory.CreateDirectory(this.SettleFileDirectory + this.BankName);
                ...
                this.SettlementFileName = this.SettleFileDirectory + this.BankName + @"\MERITUS.WELLS.NACHA." + RecordType + processDate.ToString("yyyyMMdd") + ".ACH";
                ...
                int DetailCount = 0; // data.SelectLastTraceNumber(this.BankID);
