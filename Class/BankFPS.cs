...
{
    /// <summary>
    /// Summary description for BankFPS.
    /// </summary>
    public class BankFPS : Bank
    {
        public BankFPS() { }

        public BankFPS(string bank, int bankid)
            : base(bank, bankid)
        {
            ...
            this.SettlementFileName = this.SettleFileDirectory + bank + @"\NP337175.U01";
            ...
            try
            {
                ...
                while (dr.Read())
                {
                    if (LastBatchID != DataLayer.Field2Int(dr["BatchID"]) || LastBatchFileID != DataLayer.Field2Int(dr["FileBatchID"]))
                    {
                        ...
                        batch.OriginatingDFI = this.ImmediateOrigin.Equals(string.Empty) ? string.Empty : this.ImmediateOrigin.Substring(1, 8);
