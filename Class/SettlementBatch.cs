...
namespace AchSystem
{
    public class SettlementBatch : NachaBatch
    {
        ...
        public SettlementBatch(NachaFile file, ILogging log)
        {
            this.AchFile = file;
            this.RecordTypeHeader = "5";
            this.ServiceClassCode = "200";
            ...
            this.NachaBatchLog = log;
        }

        public override void AddDetail(NachaDetail detail)
        {
            NachaBatchLog.InfoFormat("Adding Detail: Trans Type: {0}, TransRoute: {1}, Account No.: {2}, Amount: {3}, Identification No.: {4}, Account Name: {5}, TraceNumber : {6} ", detail.TransType, detail.TransRoute, detail.AccountNo, detail.Amount, detail.IdentificationNumber, detail.AccountName, detail.TraceNumber);
            ...
            this.EntryHash += Convert.ToInt64(detail.TransRoute.Substring(0, 8));
        ...
        {            
            if (this.AchFile.Bank.BankID == (int)AchBankInfo.FDR) //FDR
                this.OriginatingDFI = "12203936";
