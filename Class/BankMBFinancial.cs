...
namespace AchSystem
{
    ...
    /// </summary>
    public class BankMBFinancial : Bank
    {
        public BankMBFinancial() { }

        public BankMBFinancial(string bank, int bankid)
            : base(bank, bankid)
        {
        ...
        public override bool CreateSettlementFile(DateTime Dt)
        {
            ...
            //     for (var i = 2; i > 0; i--)
            //     {
            //         CreateSettlementFile(DateTime.Now.AddDays(-i), cuttOffTime.AddDays(-i - 1)); 
            //         cuttOffTime = DateTime.Now;
            //     }
            // }
            // // If this action is not  on Monday, the second parameter always will be avoid it by the SPs
            //return CreateSettlementFile(DateTime.Now, DateTime.Now.AddDays(-1));
        ...
        public bool CreateSettlementFile(string RecordType, DateTime processDate, DateTime cuttOffTime)
        {
            try
            {
                ...
                this.SettlementFileName = this.SettleFileDirectory + this.BankName + @"\MERITUS.MBF.NACHA." + processDate.ToString("yyyyMMdd") + ".ACH";
