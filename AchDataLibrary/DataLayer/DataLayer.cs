...
namespace Nmc.Ach.Dal
{
	...
	public class DataLayer
	{
        ...
        public const int m_CommandTimeout = 1000;
        ...
        public static string ConnectStringBuild()
        {
            ...
            //return ConfigurationManager.AppSettings["ConnectSQLAch"].ToString();

        }

        public static string ConnectStringBuildPrimus()
        {
            Crypto crypto = new Crypto();
            string conn = string.Empty;
            conn = "Data Source=" + ConfigurationManager.AppSettings["MerchantDB_Server"].ToString() + ";";
            conn += "Initial Catalog=" + ConfigurationManager.AppSettings["MerchantDB_Database"].ToString() + ";";
            conn += "User ID=" + crypto.Decrypt(ConfigurationManager.AppSettings["MerchantDB_UserID"].ToString()) + ";";
            conn += "password=" + crypto.Decrypt(ConfigurationManager.AppSettings["MerchantDB_Password"].ToString()) + ";";
        ...
        public static string ConnectStringTransDB()
        {
            Crypto crypto = new Crypto();
            string conn = string.Empty;
            conn = "Data Source=" + ConfigurationManager.AppSettings["TransDB_Server"].ToString() + ";";
            conn += "Initial Catalog=" + ConfigurationManager.AppSettings["TransDB_Database"].ToString() + ";";
            conn += "User ID=" + crypto.Decrypt(ConfigurationManager.AppSettings["TransDB_UserID"].ToString()) + ";";
            conn += "password=" + crypto.Decrypt(ConfigurationManager.AppSettings["TransDB_Password"].ToString()) + ";";
            //conn += ConfigurationManager.AppSettings["MerchantDB_Attributes"].ToString() + ";";
        ...
        public static DataSet GetDataSetPaging(string SQL, string ConnectionString,int StartRecord,int MaxRecord)
        {
            SqlDataAdapter da;
            DataSet ds;
            SqlCommand cmd = new SqlCommand();
        ...
        public static DataSet GetDataSetPaging(SqlCommand cmd, string ConnectionString, int StartRecord, int MaxRecord)
        {
            SqlDataAdapter da;
            DataSet ds;
		...
		public static DataSet GetDataSet (SqlCommand cmd, string ConnectionString)
		{     
			SqlDataAdapter da; 
			DataSet ds;
		...
		public static int ExecuteSQL(string SQLStatement, string ConnectionString)
		{
			...
			{
                if (e.Number == 547)
                    throw new Exception("This record is related to other records in the database.  Please delete the related records first");
                else
                    throw e;
            }
			finally
        ...
        public static string ExecuteScalar(string SQLStatement, string ConnectionString)
        {
            SqlCommand cmd = new SqlCommand();
            string strReturnValue = string.Empty;
            try
        ...
        public static string Field2StrQuoted(object obj)
        {
            if (obj == DBNull.Value)
                return (char)34 + string.Empty + (char)34;
            else
        ...
        /// <summary>
        /// Be careful! if dbnull, this returns a non-deterministic value
        /// </summary>
