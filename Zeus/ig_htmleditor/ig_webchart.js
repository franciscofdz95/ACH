...
function IGBrowser() 
{
	...
	// N.A. 2/24/2016 Bug #211057: Include real IE11 check.
	this.IsIE11Plus         = this.AgentName.indexOf("rv:") != -1 && parseFloat(this.AgentName.substr(this.AgentName.indexOf("rv:") + 3)) >= 11 && this.IsIE;
	this.IsIEPlus           = this.IsIE9Plus || this.IsIE11Plus;
	// N.A. 2/24/2016 Bug #211057: Exclude IE11 or later in IsNetscape6 check.
	// I.G. 07/13/2017 Bug #238984: Exclude Firefox in IsNetscape6 check.
	this.IsNetscape6		= !this.IsSafari && (this.IsDom && navigator.appName == "Netscape") && !this.IsIE11Plus && !this.IsFirefox;
	this.IsWin              = ((this.AgentName.indexOf("win") != -1) || (this.AgentName.indexOf("16bit") != -1));
	this.IsXHTML            = document.compatMode=="CSS1Compat";
	this.ID					= "IGB";	
	// MV 11/29/17 #244578 we need to check and for Chrome to fix this
	this.IsChrome			= window.navigator && window.navigator.vendor === "Google Inc.";
...
function NewMouseDown(evt) 
{
	...
	// I.G. 09/18/2017 Bug #241708 Tooltips do not display in Firefox. (There is no window.event API so use evt)
	if (IGB.IsFirefox)
	{
...
function NewMouseMove(evt) 
{
	...
	if( EngagedObject!=null) 
	{
		...
		{
			// I.G. 09/18/2017 Bug #241708 Tooltips do not display in Firefox.
			if (IGB.IsFirefox)
			{
...
function IGUltraChart(id, imageUrl, uniqueId)
{
	...
	this.onmousemove = function(evt, id) {
	    ...
	    if (IGB.IsFirefox) {
			...
			// I.G. 07/17/2017 Bug #238692 When the chart is added at run time and the page is scrolled tooltips display in the wrong location.
			// MV 11/29/17 #244578 Same fix as for #238692 for Chrome.
	        if ((IGB.IsIE8Plus || IGB.IsIE11Plus || IGB.IsOpera9Plus || IGB.IsChrome) && document.documentElement) {
	...
	// I.G. 09/18/2017 Bug #241708 Tooltips do not display in Firefox. (There is no window.event API so use evt)
		if (evt.type == "mousemove" && (IGB.IsFirefox || IGB.IsMac || IGB.IsOpera || IGB.IsSafari || IGB.IsIEPlus))
		{
