...
function ig_CreateWebDayView(props)
{
	if(!ig_WebControl.prototype.isPrototypeOf(ig_WebDayView.prototype))
	{
		...
		{
		    return parseInt(this._props[8]);
		}
		ig_WebDayView.prototype.setScrollPosition = function (value, evt) {
		    // M.V. 04.04.18 TFS251504
		    //  if browser's zoom level changes scrollTop could be float and we need to parse it to int
		    var parsedValue = parseInt(value);
		    this._props[8] = parsedValue;
		    this.updateControlState("ScrollPosition", parsedValue);
		    if (!evt)
		        this._divScroll.scrollTop = parsedValue;
		}
		...
		ig_WebDayView.prototype._onResize = function(type)
		{
			...
			// N.A. 08/12/2016 Bug #222296: In IE height should be calculated the same way as in the other browers.
			else if(height0.indexOf("%") > 0 && height > 0 && bot)
			{
				if(!total || total < 2)
			...
			if(!width)
			{
				...
				if(width < 10)
				{
					...
					// N.A. 3/13/2017 Bug #227142: Columns should always fill 100% of the parent container, so that when control is resized the columns are also resized correctly.
					if (sw > cw) {
						sw = cw;
					}
					if(cw && sw)
    ...
    ig_WebDayView.prototype._loadJSON = function(json, scrollPosition)
    {
		...
		this._divScroll.scrollTop = parseInt(scrollPosition);
    }
